...
function buildLendingInstanceArguments(offer: OfferDetails): Record<string, string> {
  return {
...
export async function recoverLenderPortfolioScripts(scope: string): Promise<string[]> {
  const attempts = (await loadBroadcastManifestAttempts(scope)).filter(
    attempt => (attempt.attemptKind ?? 'accept-offer') === 'accept-offer',
  )
